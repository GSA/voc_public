  ...
  $("#survey_version_id")
    .change(function() {
      ...
      if (survey_version_id == "0") {
        ...
        getSurveyDisplayTable($("#survey_version_id")
          .val());
      }
    });
        ...
        .modal({
          onClose: function(dialog) {
            ...
            $.ajax({
              ...
              type: "PUT",
              data: "display_field_value[value]=" + $("#display_field_value_value")
                .val(),
              success: function() {
                getSurveyDisplayTable($("#survey_version_id")
                  .val());
...
function setSurveyVersionSelect(survey_version_id) {
  $("#survey_version_id option[value='" + survey_version_id + "']")
    .attr('selected', 'selected');
}

function refreshSurveyResponseTable() {
  $("#survey_response_list")
    .html("Refreshing table...");
  getSurveyDisplayTable($("#survey_version_id")
    .val());
}
...
function getSurveyVersionList(survey_id) {
  var jqxhr = $.getJSON("/surveys/" + survey_id + "/survey_versions/", {
    ajax: 'true'
  }, function(j) {
    var options = '';
